...
const allImages = [...document.querySelectorAll('.href_img')];

$(document).on('click', '.href_img', function (event) {
    ...
    $.ajax({
        type: "POST",
        url: '/api-img/api-img-counter?id=' + id,
        data: { 'id': id },
    ...
    $(this).ekkoLightbox({
        ...
        onShown: function () {
            ...
            window.history.pushState({ page: `${id}` }, `title ${id}`, `?id=${id}`)


        },
        onNavigate: function (direction, itemIndex) {
            const id = allImages[itemIndex].getAttribute('data-id')
            window.history.pushState({ page: `${id}` }, `title ${id}`, `?id=${id}`)
        }
    });
});
...
const search1 = search.replace('?', "");

const arraySerch = search1.split('=');

const imageID = +arraySerch[1];



$('.href_img[data-id="' + imageID + '"]').click()

/*$(function() {
    $('.href_img').click(function(){
